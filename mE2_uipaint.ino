...
void uipaint_headerSettings(boolean full_repaint) {
  ...
  //EDIT SH
  uicore_getShortString( 99, 5); // PAN
  
    
  byte cnt;
  
  // loop all possible menu screens 
  // 2015-02-11 CH Change created second page with PANO settings
    ...
    if (
      ...
      switch (screen_code) {
        ...
        case 104:    str_len =strlen(string_97_long) * tft.getFontXsize();        // Triggers
                     ...
                     break;
//Edit SH
        case 105:    str_len =strlen(string_99_long) * tft.getFontXsize();        // Panorama
                     tft.print(string_99_long, display_width - str_len - 4, 1);    
                     break;
      }    
      ...
      if (screen_code == i) {  
        //2015-02-11 CH First Screen Selection Item
        ...
        } // end if First Screen Selection
        
        //2015-02-11 CH Second Screen Selection
        if (screen_code >=105){
        full_repaint = true;  
        tft.setColor(color_schemes[uicore_col_scheme].font_bg_selected);
        tft.fillRoundRect((cnt-5)*64, 25, (cnt-4)*64, header_height - 10);
                
        tft.setBackColor(color_schemes[uicore_col_scheme].font_bg_selected);
        tft.setColor(color_schemes[uicore_col_scheme].font_selected);
        
        // mE-Clear
        if (uicore_font_index == 0) tft.print(lines[cnt], ((cnt-5)*64)+9 , 30);
        // Sinclair
        if (uicore_font_index == 1) tft.print(lines[cnt], ((cnt-5)*64)+11 , 30);
        // Arial
        if (uicore_font_index == 2) tft.print(lines[cnt], ((cnt-5)*64)+10 , 30);
        } // end if Second Screen Selection
        
      } else {

        //2015-02-11 CH First Screen Items
        if (screen_code <=104){ 
        tft.setColor(color_schemes[uicore_col_scheme].background_header);
        ...
        // Arial
        if (uicore_font_index == 2) tft.print(lines[cnt], (cnt*64)+10 , 30);
        } // end if First Screen Items

        //2015-02-11 CH Second Screen Items
        if (screen_code >=105){ 
        tft.setColor(color_schemes[uicore_col_scheme].background_header);
        tft.fillRect((cnt-5)*64, 25, (cnt+1)*64, header_height - 10);
